...
Mills Lagrangian. This is covered in \secref{sec:continuum_gauge}, which we
follow up in \secref{sec:group_intro} with a brief overview of basic group
...
lattice. In the \secrefs{sec:fermion_doubling,sec:scale_setting} we review the
...
$\phi_i$. There is an analogue between the two theories and one can convert
between them by Wick rotating the time coordinate $x^0_E = -i x^0_M$. Whenever
the possibility for confusion exists we use an $M$ to symbolise Minkowski space,
the world of QFT, and an $E$ to symbolise Euclidean space, the world of SM.
...
symmetry, and signals the existence of helicity eigenstates. The existence of the
...
\end{equation}
%
These we ignore in the current study as they contribute to the breaking of other
...
\end{equation}
%
where the sum goes over the discrete points on the lattice, and we have used the
...
represented as a higher dimensional tensor or with a super index describing, all
degrees of freedom.
...
transformation property, and reproduces the Dirac Lagrangian in the continuum, it
...
discretisation schemes being \emph{overlap fermions} \citep{Neuberger:1997fp,Neuberger:1998wv},
...
configuration depend on the $n$'th configuration through the Markov chain,
