...
\end{equation}
%
The transporter $\Lambda$ is exactly a member of the representations on $G$ as
it transforms between the bases at the different points. It therefore transforms
as
%
\begin{equation}
...
%
\begin{equation}
  \mathcal{S}_f = a^4 \sum_x \frac{1}{2a}\sum_{\mu=1}^4 
    \Big( \bar{\psi}(x+\mu)U_{\mu}(x) 
    - \bar{\psi}(x-\mu) U_{\mu}^{\dagger}(x)  \Big) \psi(x) 
    + m \bar{\psi}(x)\psi(x),
\end{equation}
%
...
later. Including the gauge field is even easier as we can use the plaquette
transporter we defined in \meqref{eq:transport_plaquette}. This defines the
Wilson gauge action
%
\begin{equation}
