...
\end{equation}
%
which only give the same result due to \meqref{eq-gauge-link-integral-graphic}.
Higher order corrections can be constructed in a similar manner. We will see
that this means that most of the finite $\beta$ corrections to the effective
theory can also be implemented in terms of $\kappa$ corrections by simply
replacing the plaquettes by a sufficiently long fermion loop with the same
geometric border.
...
\end{multline}
%
When we calculated the static determinant, we saw that every loop in the
temporal direction came with a weight $h_1(z,\kappa)$. After carrying out the
spatial gauge links, all the terms in \meqref{eq-static-loop-gauge-corr} result
in an untraced Polyakov loop, and it is therefore natural to absorb these terms
into a shift in $h_1$ \citep{Fromm:2011qi,Christensen:2013xea}
%
\begin{equation}
  h_1(z,\kappa,\beta) = h_1(z,\kappa) \exp \bigg( 6 N_t \kappa^2 u \bigg(
    \frac{1-u^{N_t-1}}{1-u} + 4 u^4 - 12\kappa^2 + 9\kappa^2 u + 4 \kappa^2 u^2 -
    4\kappa^4\bigg) \bigg)
\end{equation}
