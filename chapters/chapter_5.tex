...
\end{equation}
%
as we showed in \chapref{chap4}. Comparing the above equation to the scalar
...
\figref{fig:binding_energy_Z2}. We see that the binding energy decrease together
with the constituent quark masses (increase $\kappa$), which is what one would
expect.  The plot on the left is to leading order in $h_2$, while the plot on
the right shows the fourth order, $h_2^4$, of the expansion. As one can see
higher order solution develops some interesting behaviour at the higher values
of $\kappa$ as we pass the $\mu_B / m_B = 1$ line. It is however beyond
convergence, and we need more orders in the effective theory before we can say
anything about the behaviour at higher chemical potential.
...
taking the $a$ derivative in such a way that $m_B$ remained constant, we choose
to vary the parameters $\kappa(a), \beta(a), N_t(a)$ in such a way that
...
the baryon mass to be $30 \: \mathrm{GeV}$. We observe that moving towards the
continuum limit not only decreases the constituent quark masses, but also
increase the number of temporal slices needed to fix temperature. These two
...
\figref{fig:numerical_convergence} (right) tells us that $h_2 \lesssim 0.04$ for
...
This would e.g. be a similar task to the one undertaken by L\"{u}scher and Weisz
in their groundbreaking  on the $\lambda \phi^4$ lattice theory
\citep{Luscher:1988gk,Luscher:1988uq} where they carried out a LCE to
$14$\textsuperscript{th} order to show the triviality of the theory. One should
however note the added difficulty due to the additional polymer interaction and
their geometry. For example the four bond LCE on a square lattice gives $17$
graphs, while our effective theory has $243$ graphs at the four bond level.

\subsection{Results} \label{sec:cluster_results}

We are now in a position to evaluate thermodynamic functions fully
...
\end{alignat}
%
where we have shortened the $\bmat{}^+$ notation for the brevity. Colour indices are
...
theory of the \emph{Self-Avoiding Walk} \citep[see e.g.][for a
 review]{Rensburg:2009jpa}. This is however not the preferred aproach as it would
spoil the nice
...
magnitude larger than its overlapping counterparts. This behaviour is due to the
...
\figref{fig:resummed_convergence_check}, where we once more plot the strong
coupling baryon number density as a function of the effective coupling constant.
In the left plot we see that we have extended the region of convergence
...
combination such as hexaquarks (dibaryon), enneaquarks (tribaryon) and
dodecaquarks (tetrabaryon). We note that the hexaquarks and dodecaquarks are
bosonic in nature, which might have an effect on the dense dynamics below
half-filling, after which the system only see the quark degrees of freedom, and
the statistics of combinations becomes irrelevant.
