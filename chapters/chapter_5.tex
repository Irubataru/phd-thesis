...
\begin{equation} \label{eq:scalar_field_Z}
  \mathcal{Z} = \int \big[\mathrm{d} \phi\big] e^{-S_0[\phi] + \frac{1}{2}
    \sum_{x,y}
    \phi(x) v(x,y) \phi(y)}, \hskip .35cm \text{and} \hskip .35cm v(x,x) = 0 \,,
\end{equation}
%
...
%
\begin{equation}
  \mathcal{Z}[J] = \int \big[\mathrm{d} \phi\big] e^{-S[\phi] + \sum_x J(x) \phi(x)}.
\end{equation}
%
...
\begin{equation}
  \frac{\delta \mathcal{W}}{\delta J(x)} \bigg|_{J=\mathrlap{0}} 
    = \frac{1}{\mathcal{Z}} \int \big[\mathrm{d} \phi\big] \, \phi(x) \, e^{-S[\phi]}
...
\end{equation}
%
which in turn define the free $n$-point functions
%
\begin{equation}
...
\end{equation}
%
It was shown in \citep{Pordt:1996it} that a graphical expansion for non-nearest
neighbour couplings (albeit uniform) can be reordered in such a way that the
class of graphs is identical to the nearest neighbour case, and thus the
procedure is in practice more general than the one stated here.
Regardless, choosing nearest neighbour interactions the grand canonical
...
\end{equation}
%
where $q$ is $2 d$ for a $d$-dimensional square lattice. The prefactor of a term
that arise from the spatial sum is called the embedding number\footnote{Also
  referred to as the \emph{lattice constant} in the lattice community.} of a
graph onto the lattice, and will differ from lattice to lattice. For every graph
arising from an action with a uniform two-point interaction, the sum over the
symbols / coordinates will yield $N$, the number of lattice sites, times the
(free) embedding number of the graph. It might seem wrong at first to include
all possible embeddings, as e.g.  the $q^2$ in the $M_1^2 M_2$ term will
naturally include the embedding corresponding to the $M_2^2$ term. This is
however not a problem, as the $M_n$ factors are given in terms of cumulants, and
consulting \secref{sec:moments_cumulants} we see that the system resolves these
issues automatically.  The embedding number is dependent on the lattice, so that
e.g.  the three bond graph
%
\begin{equation}
  \begin{tikzpicture}[graph style]
    \node[skeleton node] (n1) {};
...
%
\begin{equation}
  \mathcal{Z}_2 = \int \big[ \mathrm{d} U \big]_0 \det \qstat^{N_f} \exp \bigg(-
...
\meqref{eq:scalar_field_Z}, we see that there is an approximate one-to-one
...
\begin{subequations}
\begin{alignat}{99}
  \centermathcell{z_{0}} &= &&\int \mathrm{d} W \det \qstat^2 \,
    &&= 1 + 20h_1^3 + 50 h_1^6 + 20 h_1^9 + h_1^{12}, \\
  \centermathcell{z_{(11)}} &=  &&\int \mathrm{d} W \det \qstat^2 \,
    W_{11} &&= 15 h_1^3 + 75 h_1^6 + 45 h_1^9 + 3 h_1^{12}, \\
  \centermathcell{z_{(11)^2}} &= &&\int \mathrm{d} W \det \qstat^2
    \, W_{11}^2 &&= 6 h_1^3 + 95 h_1^6 + 96 h_1^9 + 9 h_1^{12}, \\
  \centermathcell{z_{(11)^3}} &= &&\int \mathrm{d} W \det \qstat^2
    \, W_{11}^3 &&= h_1^3 + 90 h_1^6 + 188 h_1^9 + 27 h_1^{12}, \\
  \centermathcell{z_{(11)^4}} &= &&\int \mathrm{d} W \det \qstat^2
    \, W_{11}^4 &&= 60 h_1^6 + 312 h_1^9 + 81 h_1^{12}.
\end{alignat}
\end{subequations}
%
...
\begin{equation} \label{eq:lce_z_definition}
  z_{(n_1 m_1)^{k_1}\cdots(n_p m_p)^{k_p}} = \int \mathrm{d} W \det \qstat^{N_f}
...
notice the first term to be somehow related to the rest energy of the system.
...
enough from the parameter range in this study for the two to be interchangeable.
...
thus far unspecified, lattice spacing. However, for us to be able to make any
...
continuum limit not only increases the hopping parameter, but also
...
N_t / N_c$, as is clearly shown in \figref{fig:a_of_beta} (right).
...
determined by the convergence limit of our theory. For instance the convergence
plot \figref{fig:numerical_convergence} (left) tells us that $h_2 \lesssim
0.04$ for $\mathcal{Z}_2$ if we demand at most $10 \%$ deviation. If we fix
$m_B = 30 \:\mathrm{GeV}$ and $T = 10 \:\mathrm{MeV}$, we find an upper limit
for $\beta \lesssim 6.02$. Afterwards, the scaling of the observable with
respect to the lattice spacing is analysed
%
\begin{equation}
...
that we still have $\mathcal{O}(a^2)$ remnants in the observables.  The
observables are then fitted to the quadratic function, and the continuum limit
results are extracted from an extrapolation to $a=0$. The fit is carried out
with two sets of data points, which are chosen based on the observable and the
value for $\mu$. A common choice here is $\Delta \beta = 0.01$ and then using
the final $10$ and $6$ points respectively for the two fits. One then averages
over the two values, and takes the difference as an error estimate for the
systematic error. The importance of a data point for the fit is weighted by the
difference in the computed observable at a fixed lattice spacing for two
consecutive orders. The fitting procedure is illustrated in
\figref{fig:continuum_extrapolation} (left). In the right hand plot is the
continuum extrapolated baryon number density of the $\mathcal{Z}_2$ partition
function expanded to four bonds using the linked cluster expansion.

\begin{figure}
  \begin{center}
    \begin{adjustbox}{max width=0.95\textwidth}
      \includegraphics{section_2/nb_of_a_Z2}
...
\begin{multline} \label{eq:multi_point_scalar_Z}
  \mathcal{Z} = \int \big[\mathrm{d} \phi_i\big] \exp \bigg(-S_0[\phi_i]
  +\frac{1}{2!} \sum_{x,y} \sum_{i,j} v_{ij}(x,y) \phi_i(x) \phi_j(y) \\
  +\frac{1}{3!} \sum_{x,y,z} \sum_{i,j,k} u_{ijk}(x,y,z) \phi_i(x) \phi_j(y)
  \phi_k(z) + \dots \bigg).
\end{multline}
%
We introduce sources $J_i$ similarly to how they were introduced in
\secref{sec:classical_lce_nn}. This gives us a linked cluster expansion for the
...
%
\begin{multline}
  \mathcal{Z}_4 = \int \big[ \mathrm{d} U \big]_0 \det \qstat^{N_f} \exp \bigg(-2
...
\begin{equation}
  ...
  e^{-S_0[\phi]} \Leftrightarrow \mathcal{J} (U_0, W_{11}, W_{21}) \det \qstat^{N_f} \:.
\end{equation}
%
...
function in the thermodynamic limit through $\mathcal{O}(\kappa^8 u^5)$ to first order in
...
\begin{figure}
  ...
  \caption{Convergence of the analytic expressions in $h_2$ (left) and $\beta$
    (right) overlaid with the simulation results at highest order. The
    normalised baryon number density has been used in the right plot.}
...
\end{equation}
%
so that its Maclaurin series agrees with the original series to
...
different Pad\'e approximants, as the $L$-$M$ ratio can be set freely. It is
...
rate of the Pad\'e approximants is far superior to that of the pure linked
...
\emph{silver blaze} problem, namely that it is zero up until onset transition, where it
...
express $G_{n-1}$ in terms of $G_{n-2}$
%
\begin{equation}
...
\begin{multline}
  ...
  \bmat{}_{\vec{x}_0}^{ab}(t_1,t_2) \bigg(\frac{2 \kappa^2}{N_c}\bigg)^n
    ...
    \sum_{\substack{i_0,i_1,\\\dots,i_n}}
      W_{11}(\vec{x}_{n+1}) \prod_{k=1}^{n} \big(-W_{21}(\vec{x}_k)\big) \\
  =\bmat{}_{\vec{x}_0}^{ab}(t_1,t_2) \bigg(\frac{2 \kappa^2}{N_c}\bigg)^n 
  N_{t}^{n-1} \sum_{\substack{i_0,i_1,\\\dots,i_n}}
    W_{11}(\vec{x}_{n+1}) \prod_{k=1}^{n} \big(-W_{21}(\vec{x}_k)\big)\;. \label{eq:full_gn}
\end{multline}
%
...
%
\begin{multline}
  \int \big[\mathrm{d} U\big]_{\mu} \, \mathcal{C}_n^* =\\
   \scalemath{0.85}{\frac{1}{g}\binom{N-1+n}{n}} \Omega \int \big[\mathrm{d} U\big]_0 \, \det \qstat^{N_f}
    \tr \big[ \bmat{}_1 M_1 \bmat{}_2 M_2 \cdots \bmat{}_N M_N  \big] \\
  \times \bigg(\frac{2d \kappa^2}{N_c}\int \big[\mathrm{d} U\big]_0 \,
    \det \qstat^{N_f} \,W_{11}\bigg)^N
  \bigg(\minus{}\frac{2d \kappa^2N_{\tau}}{N_c}\int \big[\mathrm{d} U\big]_0 \,
      \det \qstat^{N_f} \,W_{21}\bigg)^{\mathrlap{n}}\;.
\end{multline}
%
...
%
\begin{multline}
  \sum_{n=0}^{\infty} \int \big[\mathrm{d} U\big]_{\mu} \mathcal{C}_n^*
    = \frac{1}{g} \int \big[\mathrm{d} U\big]_{\mu} \bar{\mathcal{C}}_0^*\:
    \Big(\frac{(2d) \kappa^2}{N_c} z_{(11)}\Big)^N
    \sum_{n=0}^{\infty} \scalemath{0.9}{\binom{N-1+n}{n}} \big(-(2d) h_2\, z_{(21)}\big)^n \\
  = \frac{1}{g} \int \big[\mathrm{d} U\big]_{\mu} \bar{\mathcal{C}}_0^*\:
...
the resummation is shown in \figref{fig:resummation_demonstration} (left), where
we have plotted the contribution to the baryon number density of the single
chain, single graph terms. We plot the full result up to the highest known order as
well as higher order truncated results from the resummation formula, and the
resummed result itself. We see that the different orders diverge to $\pm \infty$
at some point at increasing values of $h_2$ while the full resummed result stays
under control.
...
\end{equation}
%
Every cut in the chain will replace a $-h_2 W_{21}$ in the expression by a
...
nearest neighbours to the root position.  Finally one has to evaluate the colour
...
effort should be focussed on structural improvements rather than systematical.
...
$a$-scaling of the curves at greater values of $\mu$. We see that they split
further and further apart as we increase $\mu$ due to saturation, which in turn
makes the continuum extrapolation more challenging. This is also
...
\begin{figure}[t]
  \begin{center}
    ...
    \end{adjustbox}
  \end{center} \vskip -.5cm
  \caption{Equation of state for the heavy quark effective theory at two
...
for heavy QCD in the cold and dense regime. \Figref{fig:eos_plots} shows the
path traced out in a pressure vs density plot with varying chemical potential.
The finite lattice spacing paths are plotted in \ColHlIText{} and
\ColHlIIText{}, and they clearly eventually curve upwards. This is because of
saturation, and with increasing chemical potential above saturation, the density
will stay constant, while the pressure will continue to rise. The expected
asymptotes are also shown in the same plot, in matching colours to their
corresponding plot points, given by
%
\begin{equation}
...
only sees the quark degrees of freedom, and the statistics of combinations
becomes irrelevant.
...
contribute to the pressure in the cold and dense regime, however still adding
...
explored by \cite{'tHooft:1973jz}, who discovered that a simplification
...
much simpler than QCD, as well as not possessing a sign problem. On the
...
interest in higher $N_c$ from a conformal field theory perspective, as there
exists a line in the $N_f \big/ N_c$ plane that separates conformal and
non-conformal field theories, which is known as the \emph{conformal window}
...
enter analytic computations do not change as we increase $N_c$. This means that
in the strong coupling limit, all we have to do is recompute the $z$-functions
using the methods outlined in the appendices for every $N_c$. The baryon number
density for different gauge groups is plotted in \figref{fig:diff_nc_plots}. We
see that at higher and higher values of $N_c$, the transition becomes steeper
and steeper. We have defined $n_B$ by dividing out a factor of $N_c$ from the
quark number density, a colour neutral state requires at least $N_c$ quarks.
The behaviour at increasing $N_c$ can be easily seen from the static
determinants in \tabref{tab:static-determinant}.  The static determinant will
always have a fugacity coefficient raised to power $N_c$ at highest order.  This
means that the onset is exponentially suppressed by $N_c$, and the saturation
exponentially enhanced. In the limit this gives a natural transition which
matches physical intuition.  As the number of quarks needed to form a baryon
increases, it becomes more and more energetically expensive to create one. The
silver blaze phenomenon is therefore strengthened, and in the actual limit, one
will always have a first order transition to nuclear matter, independent of the
temperature. It would be very interesting to use the effective theory to find
the liquid gas endpoint as a function of $N_c$, $\kappa$ and $T$.
Similar to the fact that such an endpoint exists at a critical temperature
depending on the quark mass, it also exists for any temperature and $\kappa$ at a
critical $N_c$.  However, this can only be seen if one includes gauge
corrections, as the transition point is defined by an interplay between the
fermion-gauge interactions. We already see that our effective coupling $h_2
\propto N_c^{-1}$, and will thus become small. The gauge sector on the other
hand has graphs that do not suffer from this suppression, and will therefore
compete with the quarks.
...
\begin{equation} \label{eq:zeros_partition_function}
  \mathcal{Z}_2 = \int \big[\mathrm{d} U\big]_0 \, \det \qstat^{N_f} \prod_{\langle \vec{x},\vec{y} \rangle}
...
in \figref{fig:yang_lee_zeros}. Already at these small lattices we see an
interesting structure, which starts developing the three-fold symmetry of
SU($3$).  Along with increasing volume, we expect even more complexity, while
artificial zeros should disappear. However, due to the need for an exact
analytic expression when evaluating the zeros, the integral in
\meqref{eq:zeros_partition_function} becomes exceedingly expensive in its
evaluation, and the number of terms to evaluate in a naive approach scales as
$2^{6 N_t N_s^3}$. So, even for a $1\times{}2^3$ lattice, the number of terms
for the integral would be $\sim 2.8 \cdot 10^{14}$. Most of the terms are of
course the same, and we therefore see the need for methods of extracting the
