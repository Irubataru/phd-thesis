...
from \secref{sec:moments_cumulants} we see that the system resolves these issues
...
notice that the first term to be somehow related to the rest energy of the system.
...
function in the thermodynamic limit through $\kappa^8 u^5$ to first order in
...
always have a fugacity coefficient to power $N_c$ at highest order.  This means
that the onset is exponentially suppressed by $N_c$, and the saturation
...
the liquid gas endpoint as a function function of $N_c$, $\kappa$ and $T$.
Similar to the fact that such an endpoint exists at a critical temperature
depending on the quark mass, it also exist for any temperature and $\kappa$ at a
