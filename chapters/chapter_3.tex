...
\end{equation}
%
This gives us the final form of the Wilson action that we will make use of for
the work in this thesis
%
\begin{align}
  &\mathcal{S}_{f,w} = a^4 \sum_{x} \bigg(
    \big(m + {\textstyle\frac{4 r}{a}}\big) \bar{\psi}(x) \psi(x)
    - \frac{1}{2a} \sum_{\mu = \pm 1}^{\pm 3} \bar{\psi}(x+\hat{\mu}) (r +
    \gamma^{\mu}) U_{\mu}(x) \psi(x)\nonumber\\
  &-\frac{1}{2a} \big(\bar{\psi}(x+\hat{0}) (r + \gamma^{0}) z U_{0}(x) \psi(x)
    + \bar{\psi}(x-\hat{0}) (r - \gamma^{0}) z^{-1} U_{0}^{\dagger}(x-\hat{0}) \psi(x)\big)
    \bigg)
\end{align}
