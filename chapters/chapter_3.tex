...
This chapter give a short overview of some aspects of statistical mechanics
...
Several volumes of the standard literature have been consulted with respect to
...
This limitation is sketched in \figref{fig:saturation_sketch} where a stereotypical
...
integrals of finite chemical potential systems through Monte Carlo methods. We
saw that we can use the probability density
%
\begin{equation}
...
to as the \emph{sign problem}, and the issue is not exclusive to lattice studies
of fermionic systems at finite density. Being categorised as an NP-hard problem
...
expansions, and analytic continuation from imaginary chemical potential; there
is also stochastic quantisation, which is not a Monte Carlo method.  Although
none of these procedures actually tackle the essence of the sign problem, with
sufficiently powerful computers results with reasonable accuracy can be
obtained. However, one should take great care to sample correctly if one wants
sensible results, as was demonstrated in \citep{Osborn:2008eg}.
...
Another method that has been given much attention recently is simulating at purely
...
(\figref{fig:phase_diags}, right), where one can compute the curvature of this
surface upon crossing into the region of real chemical potential
...
The sign problem manifests itself in the complex flow of the fields. For large
values of the chemical potential, the field configurations will spend a
...
\citep{Seiler:2012wz}, which is the application of invariant transformations on
the variables that push them closer to non-complex field values. For QCD it
has been shown that this places a restriction on the coupling strengths for
which the method is applicable. A recent comparison made between the reach of
reweighting to that of stochastic quantisation shows that they are comparable
\citep{Fodor:2015doa}.  The newest developments can be found in
\citep{Aarts:2016qrv}.
...
to the wrong theory, we study a \emph{Random Matrix Theory} (RMT), which is
